...
```bash
clio-kit mcp-server compression
```
...
```sh
claude mcp add compression-mcp -- clio-kit mcp-server compression
```
...
uv --directory=$CLONE_DIR/clio-kit/clio-kit-mcp-servers/compression run compression-mcp --help
```
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/clio-kit.git
uv --directory=$env:CLONE_DIR\clio-kit\clio-kit-mcp-servers\compression run compression-mcp --help
```
...
```bash
claude mcp add clio-compression -- clio-kit mcp-server compression
```
...
/plugin install clio-compression@clio-kit
```
## Claude Desktop
