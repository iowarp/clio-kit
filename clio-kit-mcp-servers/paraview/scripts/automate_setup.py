...
class SetupAutomator:
    ...
    def check_prerequisites(self):
        ...
        if not self.venv_python.exists():
            self.log(
                "Virtual environment not found. Please run 'uv venv --python 3.10' first.",
                "ERROR",
            )
            return False
        ...
        try:
            result = subprocess.run(
                [str(self.venv_python), "-c", "import fastmcp"],
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                self.log(
                    "Project dependencies not installed. Please run 'uv pip install -e .' first.",
                    "ERROR",
                )
                return False
        except Exception as e:
    ...
    def configure_paraview_build(self):
        ...
        if adios2_mpi_available:
            ...
            cmake_args.extend(["-DPARAVIEW_USE_MPI=ON", "-DADIOS2_USE_MPI=ON"])
        else:
            self.log("ADIOS2 MPI support not detected - building without MPI")
            cmake_args.extend(["-DPARAVIEW_USE_MPI=OFF", "-DADIOS2_USE_MPI=OFF"])
    ...
    def build_paraview(self):
        ...
        self.log(
            f"ParaView build and installation completed in {elapsed / 60:.1f} minutes ✓"
        )
        return True
    ...
    def check_adios2_mpi_support(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["pkg-config", "--exists", "adios2-mpi"], capture_output=True, text=True
            )
            if result.returncode == 0:
                return True
            ...
            mpi_libs = glob.glob("/usr/lib*/libadios2*mpi*") + glob.glob(
                "/usr/local/lib*/libadios2*mpi*"
            )
    ...
    def setup_python_integration(self):
        ...
        paraview_site_packages = (
            self.paraview_install / "lib" / "python3.10" / "site-packages"
        )
        venv_site_packages = (
            self.project_root / ".venv" / "lib" / "python3.10" / "site-packages"
        )
        pth_file = venv_site_packages / "paraview.pth"

        if not paraview_site_packages.exists():
            self.log(
                f"ParaView Python packages not found at {paraview_site_packages}",
                "ERROR",
            )
            return False
...
""",
            ...
            (
                "MCP Server",
                "import sys; sys.path.insert(0, '.'); from src.server import main; print('✓ MCP server importable')",
            ),
        ]
    ...
    def run_complete_setup(self, steps=None):
        ...
        self.log(
            f"🎉 Complete setup finished successfully in {total_time / 60:.1f} minutes!"
        )
...
def main():
    ...
    parser.add_argument(
        "--deps-only", action="store_true", help="Only install system dependencies"
    )
    parser.add_argument(
        "--build-only",
        action="store_true",
    ...
    if args.deps_only:
        ...
        steps = [("verify_installation", "Verifying installation")]
    else:
