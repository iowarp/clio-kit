...
class InstallationTester:
    ...
    def run_test(
        self, command, description, category="General", timeout=30, allow_fail=False
    ):
    ...
    def test_basic_environment(self):
        ...
        self.run_test(
            "python --version", "Python in virtual environment", "Environment"
        )

        # Project dependencies
        self.run_test(
            "python -c 'import fastmcp; print(f\"FastMCP v{fastmcp.__version__}\")'",
            "FastMCP dependency",
            "Environment",
        )
        ...
        self.run_test(
            "uv run install-deps --check-only", "System dependencies", "Environment"
        )
    ...
    def test_paraview_core(self):
        ...
        self.run_test(
            "python -c 'import paraview; print(f\"ParaView available at: {paraview.__file__}\")'",
            "ParaView Python package",
            "ParaView",
        )

        # ParaView Simple module
        self.run_test(
            "python -c 'import paraview.simple as pv; print(\"ParaView Simple module loaded\")'",
            "ParaView Simple API",
            "ParaView",
        )
        ...
        # ParaView version
        self.run_test(
            "python -c 'import paraview; print(f\"ParaView version: {paraview.version}\")'",
            "ParaView version info",
            "ParaView",
            allow_fail=True,
        )
    ...
    def test_vtk_adios2(self):
        ...
        # ADIOS2 Core Reader
        self.run_test(
            "python -c 'import vtk; reader = vtk.vtkADIOS2CoreImageReader(); print(\"✓ ADIOS2 Core Reader available\")'",
        ...
        self.run_test(
            "python -c 'import vtk; writer = vtk.vtkADIOS2VTXWriter(); print(\"✓ ADIOS2 VTX Writer available\")'",
            "ADIOS2 VTX Writer",
            "ADIOS2",
            allow_fail=True,
        )
        ...
        for class_name, desc in adios2_classes:
            self.run_test(
                f'python -c \'import vtk; getattr(vtk, "{class_name}"); print("✓ {class_name} available")\'',
                f"ADIOS2 {desc}",
                "ADIOS2",
                allow_fail=True,
            )
...
"""
                self.run_test(
                    f'python -c "{test_code}"', "BP5 file reading", "BP5", timeout=60
                )
            else:
    ...
    def test_mcp_server(self):
        ...
        self.run_test(
            "uv run paraview-mcp --help", "MCP server help", "MCP", allow_fail=True
        )

        # Test server import (without starting)
        self.run_test(
            'python -c \'import sys; sys.path.insert(0, "src"); import server; print("✓ MCP server module importable")\'',
    ...
    def test_paraview_binaries(self):
        ...
        if paraview_install.exists():
            ...
            if pvserver_bin.exists():
                self.run_test(
                    f"{pvserver_bin} --version",
                    "ParaView server binary",
                    "Binaries",
                    timeout=10,
                    allow_fail=True,
                )
            ...
            print(
                "   ⚠️  No local ParaView installation found (.paraview directory missing)"
            )
    ...
    def run_all_tests(self):
        ...
        critical_failures = [
            ...
            and result["category"] in ["Environment", "ParaView", "VTK"]
        ]
