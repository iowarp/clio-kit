...
def find_conda_paraview():
    ...
    try:
        ...
        if conda_prefix:
            paraview_path = (
                Path(conda_prefix) / "lib" / "python3.10" / "site-packages" / "paraview"
            )
...
def verify_adios2_support(verbose=False):
    ...
    test_commands = [
        ...
        (
            "ADIOS2 Reader",
            "import vtk; reader = vtk.vtkADIOS2CoreImageReader(); print('✓ ADIOS2 reader available')",
        ),
        (
            "ADIOS2 Writer",
            "import vtk; writer = vtk.vtkADIOS2VTXWriter(); print('✓ ADIOS2 writer available')",
        ),
    ]
