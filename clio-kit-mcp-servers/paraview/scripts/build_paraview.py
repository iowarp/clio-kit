...
def check_system_dependencies():
    ...
    if missing:
        ...
        if platform.system() == "Linux":
            print(
                "sudo apt update && sudo apt install git cmake build-essential ninja-build \\"
            )
...
def build_paraview(project_root, force_rebuild=False, parallel_jobs=None):
    ...
    if not paraview_src.exists():
        ...
        run_command(
            f"git clone --recursive https://gitlab.kitware.com/paraview/paraview.git {paraview_src}"
        )
...
def setup_python_path(project_root):
    ...
    paraview_site_packages = (
        project_root / ".paraview" / "lib" / "python3.10" / "site-packages"
    )
...
def main():
    ...
    parser.add_argument(
        "--skip-deps-check", action="store_true", help="Skip system dependencies check"
    )
    parser.add_argument(
