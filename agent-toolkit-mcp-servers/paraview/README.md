...
```bash
uvx iowarp-agent-toolkit mcp-server paraview
```
...
```json
{
  "mcpServers": {
    "paraview-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "mcp-server", "paraview"]
    }
  }
}
```
...
```sh
claude mcp add paraview-mcp -- uvx iowarp-agent-toolkit mcp-server paraview
```
...
uv --directory=$CLONE_DIR/agent-toolkit/agent-toolkit-mcp-servers/paraview run paraview-mcp --help
```
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/agent-toolkit.git
uv --directory=$env:CLONE_DIR\agent-toolkit\agent-toolkit-mcp-servers\paraview run paraview-mcp --help
```
