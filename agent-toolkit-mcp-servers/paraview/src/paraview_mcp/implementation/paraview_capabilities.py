...
class VisualizationEngine:
    ...
    def __init__(self, server_host="localhost", server_port=11111):
        ...
        self.primary_data_source = (
            None  # Renamed from original_source for differentiation
        )
    ...
    def connect(self, server_url="localhost", port=11111):
        ...
        try:
            ...
            if importlib.util.find_spec("paraview.simple") is not None:
                ...
                self.logger.warning(
                    "paraview.simple module not found. Running in simulation mode."
                )
                return False
        except Exception as e:
    ...
    def read_datafile(self, file_path):
        ...
        try:
            ...
            if not os.path.exists(file_path):
                return (
                    False,
                    f"Error: File does not exist at path '{file_path}'",
                    None,
                    "",
                )
            ...
            try:
                ...
                self.logger.info(
                    f"Loading file: {file_name} (size: {file_size} bytes, extension: {file_extension})"
                )
            except OSError:
                self.logger.info(
                    f"Loading file: {file_name} (extension: {file_extension})"
                )
            ...
            if file_extension in [".bp", ".bp5"]:
                ...
                if paraview_adios_status["has_support"]:
                    ...
                    self.logger.info(
                        f"Using ParaView's native ADIOS2 support for: {file_path}"
                    )
                    try:
                        reader = OpenDataFile(file_path)
                        if reader:
                            self.logger.info(
                                f"Successfully loaded BP5 file natively: {file_path}"
                            )
                        else:
                            self.logger.warning(
                                "Native BP5 loading returned None, attempting conversion fallback"
                            )
                            raise Exception("Native ADIOS2 loading failed")
                    except Exception as e:
                        self.logger.warning(
                            f"Native BP5 loading failed: {e}, trying conversion approach"
                        )
                        reader = None
                else:
                    self.logger.warning(
                        "ParaView lacks ADIOS2 support, using conversion approach"
                    )
                    reader = None
                ...
                if not reader:
                    ...
                    self.logger.info(
                        f"Converting ADIOS2 file using enhanced converter: {file_path}"
                    )
                    converted_file = self._convert_adios_to_vtk_improved(file_path)
                    if converted_file:
                        self.logger.info(
                            f"Successfully converted BP5 to: {converted_file}"
                        )
                        reader = OpenDataFile(converted_file)
                        file_path = converted_file  # Update path for display purposes
                    else:
                        self.logger.error(
                            "ADIOS2 conversion failed with enhanced converter"
                        )
                ...
                self.logger.info(
                    f"Detected RAW volume format, configuring reader for: {file_name}"
                )
            ...
            if not reader:
                return (
                    False,
                    f"Error: Failed to create reader for {file_path}. The file format may not be supported.",
                    None,
                    "",
                )
            ...
            try:
                ...
                self.logger.info(
                    "Successfully configured display properties and reset camera"
                )
    ...
    def get_active_source_names_by_type(self, source_type=None):
        ...
        try:
            ...
            if not result_sources and source_type:
                ...
                message = f"Found {len(result_sources)} source(s)" + (
                    f" of type '{source_type}'" if source_type else ""
                )
        ...
        try:
            ...
            if enable:
                ...
                if current_rep == "Volume":
                    ...
                    status_message = (
                        "Volume rendering hidden (representation preserved)"
                    )
                else:
    ...
    def toggle_visibility(self, enable=True):
        ...
        try:
            ...
            if not GetActiveSource():  # noqa: F405
                return False, "Error: No data selected. Load data first.", None

            view = GetActiveView()
            display = GetDisplayProperties(GetActiveSource(), view)  # noqa: F405
            ...
            # Get the source name using the helper function
            source_name = self._get_source_name(GetActiveSource())  # noqa: F405
    ...
    def color_by(self, field, component=-1):
        ...
        try:
            ...
            if rep_type in ["Outline", "Wireframe"]:
                return (
                    False,
                    f"Error: The current representation type '{rep_type}' cannot be colored by fields. Try changing to 'Surface' or 'Volume' first.",
                )
            ...
            if (
                point_info.GetNumberOfArrays() == 0
                and cell_info.GetNumberOfArrays() == 0
            ):
                return (
                    False,
                    "Error: The active source does not have any data arrays to color by.",
                )
            ...
            if not field_available:
                ...
                return (
                    False,
                    f"Error: Field '{field}' not found. Available fields are: {fields_str}",
                )
    ...
    def get_histogram(self, field=None, num_bins=256, data_location="POINTS"):
        ...
        try:
                ...
                if num_arrays == 1:
                    ...
                    for i in range(num_arrays):
                        available_arrays.append(
                            array_info_obj.GetArrayInformation(i).GetName()
                        )
                    return (
                        False,
            ...
            if nbins_prop is None:
                return (
                    False,
                    "Error: Histogram filter does not have a 'NumberOfBins' or 'BinCount' property.",
                    None,
                )
            ...
            return (
                True,
                f"Histogram computed for field '{field}' in {data_location} with {num_bins} bins.",
                histogram_data,
            )
    ...
    def edit_volume_opacity(self, field_name, opacity_points):
        ...
        try:
            ...
            if opacity_tf is None:
                return (
                    False,
                    f"Could not find an opacity transfer function for field '{field_name}'.",
                )
    ...
    def set_color_map(
        ...
        try:
            ...
            if color_tf is None:
                return (
                    False,
                    f"Could not find or create a color transfer function for '{field_name}'.",
                )
        ...
        try:
            ...
            if point_info:
                ...
                for i in range(num_point_arrays):
                    ...
                    array_name = (
                        array_info.GetName()
                    )  # Use GetName() rather than GetArrayName()
    ...
    def create_stream_tracer(
        ...
        try:
            ...
            if hasattr(base_source, "SMProxy") and hasattr(
                base_source.SMProxy, "GetXMLName"
            ):
                ...
                if point_info:
                    ...
                    for i in range(num_arrays):
                        ...
                        if components > 1:
                            ...
                            found = True
                            self.logger.info(
                                f"Automatically selected vector field: {vector_field}"
                            )
                            break
                    if not found:
                        if num_arrays > 0:
                            ...
                            return (
                                False,
                                "Error: No arrays found in the base source.",
                                None,
                                "",
                            )
                else:
                    return (
                        False,
                        "Error: Could not retrieve point data information.",
                        None,
                        "",
                    )
    ...
    def get_screenshot(self):
        ...
        try:
            ...
            self.logger.info(
                f"Screenshot saved successfully: {screenshot_path} ({file_size} bytes)"
            )
    ...
    def plot_over_line(self, point1=None, point2=None, resolution=100):
        ...
        try:
            ...
            # show data in view
            _ = Show(plot_filter, lineChartView1, "XYChartRepresentation")
            AssignViewToLayout(view=lineChartView1)

            return (
                True,
                f"Plot over line created from {plot_filter.Point1} to {plot_filter.Point2} with {resolution} points.",
                plot_filter,
            )
        except Exception as e:
    ...
    def warp_by_vector(self, vector_field=None, scale_factor=1.0):
        ...
        try:
            ...
            return (
                True,
                f"Warp by vector applied using field '{vector_field}' with scale factor {scale_factor}.",
                warp_filter,
            )
        except Exception as e:
            ...
            if cmake_cache_path:
                ...
                for flag in adios2_flags:
                    if "PARAVIEW_ENABLE_ADIOS2:BOOL=ON" in flag:
                        enable_adios2 = True
                    elif "PARAVIEW_USE_ADIOS2" in flag and (
                        "=ON" in flag or ":UNINITIALIZED=ON" in flag
                    ):
                        use_adios2 = True
                ...
                if enable_adios2:
                    ...
                    status["message"] = (
                        "ADIOS2 available but not enabled in ParaView build"
                    )
                else:
                    status["has_support"] = False
                ...
                status["message"] = (
                    "ParaView build directory not found - cannot determine ADIOS2 support"
                )
    ...
    def _convert_adios_to_vtk_improved(self, file_path):
        ...
        try:
            ...
            for i, strategy in enumerate(conversion_strategies, 1):
                strategy_name = (
                    strategy.__name__.replace("_convert_strategy_", "")
                    .replace("_", " ")
                    .title()
                )
                ...
                try:
                    if strategy(file_path, output_file):
                        if os.path.exists(output_file):
                            ...
                            self.logger.info(
                                f"Successfully converted using {strategy_name}: {output_file} ({file_size} bytes)"
                            )
                            return output_file
                        else:
                            self.logger.warning(
                                f"Strategy {strategy_name} reported success but no output file created"
                            )
                except Exception as e:
            ...
            self.logger.error(
                f"Enhanced BP5 conversion failed with exception: {str(e)}"
            )
            import traceback
    ...
    def _convert_strategy_streaming_api(self, file_path, output_file):
        ...
        try:
            ...
            with adios2.open(file_path, "r") as f:
                ...
                self.logger.info(
                    f"Found {len(available_vars)} variables: {list(available_vars.keys())}"
                )
                ...
                for var_name, var_info in available_vars.items():
                    try:
                        ...
                        self.logger.warning(
                            f"Error processing variable {var_name}: {e}"
                        )
                ...
                self.logger.info(
                    f"Successfully read data: shape={data.shape}, dtype={data.dtype}"
                )
    ...
    def _convert_strategy_direct_bp5(self, file_path, output_file):
        ...
        try:
            ...
            for var_name in list(available_vars.keys())[
                :3
            ]:  # Limit to first 3 variables
                try:
                    ...
                    if var_obj and var_obj.Shape():
                        ...
                        if step_status == adios2.StepStatus.OK:
                            ...
                            self.logger.info(
                                f"Read variable {var_name}: shape={data.shape}"
                            )
                        else:
            ...
            if data_arrays:
                ...
                return self._create_vtk_from_numpy(
                    data_arrays[main_var], main_var, output_file
                )
            else:
                return False
    ...
    def _convert_strategy_alternative_engines(self, file_path, output_file):
        ...
        for engine in alternative_engines:
            try:
                ...
                if available_vars:
                    self.logger.info(
                        f"Engine {engine} found {len(available_vars)} variables"
                    )
    ...
    def _create_vtk_from_numpy(self, data, var_name, output_file):
        ...
        try:
            ...
            self.logger.info(
                f"Creating VTK ImageData from array: shape={data.shape}, dtype={data.dtype}"
            )
