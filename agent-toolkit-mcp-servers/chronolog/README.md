...
```bash
uvx iowarp-agent-toolkit chronolog
```
...
- ChronoLog deployment (see [setup guide](https://github.com/iowarp/agent-toolkit/blob/main/Chronolog/docs/Chronolog_setup.md))
...
```json
{
  "mcpServers": {
    "chronolog-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "chronolog"]
    }
  }
}
```
...
```sh
claude mcp add chronolog-mcp -- uvx iowarp-agent-toolkit chronolog
```
...
uv --directory=$CLONE_DIR/agent-toolkit/agent-toolkit-mcp-servers/chronolog run chronolog-mcp --help
```
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/agent-toolkit.git
uv --directory=$env:CLONE_DIR\agent-toolkit\agent-toolkit-mcp-servers\chronolog run chronolog-mcp --help
```
