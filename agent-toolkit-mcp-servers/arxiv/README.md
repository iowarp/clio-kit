...
```bash
uvx iowarp-agent-toolkit arxiv
```
...
```sh
claude mcp add arxiv-mcp -- uvx iowarp-agent-toolkit arxiv
```
...
uv --directory=$CLONE_DIR/agent-toolkit/agent-toolkit-mcp-servers/arxiv run arxiv-mcp --help
```
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/agent-toolkit.git
uv --directory=$env:CLONE_DIR\agent-toolkit\agent-toolkit-mcp-servers\arxiv run arxiv-mcp --help
```
