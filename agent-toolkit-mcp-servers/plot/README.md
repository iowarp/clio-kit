...
```json
{
  "mcpServers": {
    "plot-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "plot"]
    }
  }
}
```
...
```sh
claude mcp add plot-mcp -- uvx iowarp-agent-toolkit plot
```
...
uv --directory=$CLONE_DIR/agent-toolkit/agent-toolkit-mcp-servers/plot run plot-mcp --help
```
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/agent-toolkit.git
uv --directory=$env:CLONE_DIR\agent-toolkit\agent-toolkit-mcp-servers\plot run plot-mcp --help
```
