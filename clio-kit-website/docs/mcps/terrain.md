...
### Analyze an elevation grid

Use `dem_terrain` with a supported CSV numeric grid, NPY or NPZ array. Establish
cell spacing and horizontal/vertical units before interpreting slope, aspect or
suitability. GeoTIFF requires optional rasterio support.

### Grid a point cloud

`pointcloud_read` averages x/y/z points into cells and can write a CSV DEM for
`dem_terrain`. Choose cell size deliberately and retain the source. Ground and
canopy values in one cell are averaged together; this is not ground classification.
LAS/LAZ requires optional laspy support.
