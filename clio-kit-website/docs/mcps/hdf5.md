...
Open the file before `hdf5_batch_read` or `hdf5_aggregate_stats`; their paths
refer to datasets inside the current file. Above 500 MiB, aggregate statistics
may use a strided sample. Responses label `SAMPLED` versus `FULL DATA`, give
processed/total element counts and sampled coverage, and warn that sample totals
and bounds do not describe the whole dataset. Cross-dataset aggregation is
omitted if any result is sampled. Streaming stops at `max_chunks`, and multidimensional chunks
slice the first axis. Check memory requirements, sampling and processed coverage
before treating the result as an exact whole-dataset statistic.
