...
### Inspect a waveform archive

Use `inspect_archive` on a SAC file or TAR archive, then
`compute_trace_statistics` for sample counts, timing and amplitudes. Use
`plot_traces` to inspect waveform shape. Its normalized traces do not preserve
relative amplitude between stations; calibration and units matter.

### Analyze an event catalogue

`analyze_sequence` and `plot_sequence` accept a saved earthquake catalogue,
not raw waveforms. Report event count, magnitude completeness and b-value
uncertainty. Waveform files do not become an event catalogue through these tools.
